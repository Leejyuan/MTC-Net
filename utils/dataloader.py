...
#                                 [0.229, 0.224, 0.225])
        ...
        self.gt_transform =to_long_tensor
#        transforms.Compose([
#            transforms.ToTensor()])
...
#        gt = torch.zeros(( 1, gt.shape[1], gt.shape[2])).scatter_(0, gt.long(), 1)
        return image, gt
...
def get_loader(image_root, gt_root, batchsize, shuffle=True, num_workers=1, pin_memory=True):
...
#        for i in range(150):
        ...
        gt = to_long_tensor(gt)#torch.zeros((1, gt.shape[1], gt.shape[2])).scatter_(0, gt.long(), 1)
        self.index += 1
